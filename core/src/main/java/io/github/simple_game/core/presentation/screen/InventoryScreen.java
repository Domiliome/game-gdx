...
public class InventoryScreen extends ScreenAdapter {
    ...
    private Label descLabel; // Вернули поле ссылки на панель описания
    ...
    public void show() {
        ...
        Label.LabelStyle textStyle = new Label.LabelStyle(new BitmapFont(), Color.WHITE); textStyle.font.getData().setScale(1.6f);
        TextButton.TextButtonStyle btnStyle = new TextButton.TextButtonStyle(); btnStyle.font = new BitmapFont(); btnStyle.font.getData().setScale(2.0f); btnStyle.fontColor = Color.WHITE; btnStyle.up = new TextureRegionDrawable(btnBg);
        ...
        for (int i = 0; i < inv.getBackpack().size; i++) {
            ...
            // ВОССТАНОВЛЕНО: При обычном клике выводим имя и описание предмета вниз
        ...
        // ВОССТАНОВЛЕНО: Текстовая панель описания выбранного предмета
